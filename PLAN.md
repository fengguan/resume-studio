...
**2026-09-08 交互审阅扩展：**开发前已将原有版本保存为 Git 提交 `fa07ab8`，后续在 `feature/interactive-review` 分支开发。应用现增加按原文位置的结构预览、行内差异、段落选择与疑点定位、手动编辑及撤销、逐段 AI 对话和即时改写、草稿恢复。AI 续改仍复用原生 API，执行规则与独立段落审校；正式输出再执行全文审校。草稿独立保存在 `workspace.json`，原导出版本保持不变。浏览器预览负责交互与定位，不替代 Word 的分页与格式验收。后文为原规划基线，以 README 的当前使用方式为准。
...
**已确认的本地运行方式：**界面、解析、文档生成和文件存储在本机，支持 OpenAI、Gemini、Claude 和 DeepSeek 云端模型 API。必要的简历正文和职位内容会发送给所配置的模型服务，不能称为“数据全程不出本机”。完全离线适配保留为未来选项。
...
已确认支持 OpenAI、Gemini、Claude 和 DeepSeek 云端 API。其余按下列默认值推进：简历跟随输入语言、comment 用中文、保留原版式、当前样本以两页为目标、先单职位运行、缺失事实进入建议且不自动补入正文。
